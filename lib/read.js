...
function coalesceAndCompute(data, opts, callback) {
  var self = this;
  var baseLevelCompute = opts.computeLevels === -1 ? emptyCompute : this.compute.bind(this);
  if (data.__sm_ts) return baseLevelCompute(data[opts.varName], callback);
  ...
  async.map(data, function(data, callback) {
    ...
    async.parallel([ collectComps, collectIncludes ], function(err) {
      ...
      if (opts.restrictToComp && !Array.isArray(opts.restrictToComp))
        ...
        baseLevelCompute(node, callback);
    });
  }, function(err, data) {
    if (err) return callback(err);
...
module.exports = {
  read: function(node, opts, callback) {
    ...
    this.db.query(query, opts.params, function(err, data) {
      if (err) {
        var excString;
        if (err.neo4jException) excString = err.neo4jException;
        else if (err.fields && err.fields[0] && err.fields[0].code) excString = err.fields[0].code;
        else if (err.code) excString = err.code;

        if (typeof excString == 'string' && excString.match(/EntityNotFound/)) callback(null, false);
        else callback(err);
